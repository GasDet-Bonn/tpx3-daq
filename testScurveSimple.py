...
def run_test_pulses():
    ...
    data = chip.write_tp_pulsenumber(100, write=False)
    chip.write(data, True)
    # Get the data, do the FPGA decode and do the decode ot the 0th element
    # which should be EoC (header: 0x71)
    ...
    # Step 4a: Set VTP_coarse DAC (8-bit)
      ...
      #tpf = 64
        ...
        if thrs[len(thrs)-1]<150:
          f=open(('ThresholdScan-{}-{}.txt').format(tpc,tpf),'w')
          for index in range(512):
            if index <len(thrs):
              f.write(str(thrs[index]) + " " + str(evCounters[index])+" "+str(tpc)+" "+str(tpf) + "\n")
            else:
              f.write(str(0) + " " + str(0)+" "+str(tpc)+" "+str(tpf) + "\n")
          f.close()
