...
class ThresholdScan(ScanBase):
    ...
    def scan(self, VTh_fine_start=100, VTh_fine_stop=200, n_injections=100, mask_step=8, **kwargs):
        '''
        ...
        self.chip.set_dac("VTP_fine", 256)
    ...
    def analyze(self):
        ...
        with tb.open_file(h5_filename, 'r+') as h5_file:
            ...
            VTh_fine_start = [int(item[1]) for item in run_config if item[0] == 'VTh_fine_start'][0]
            VTh_fine_stop = [int(item[1]) for item in run_config if item[0] == 'VTh_fine_stop'][0]

            param_range = range(VTh_fine_start, VTh_fine_stop)
    ...
    def plot(self):
        ...
        with tb.open_file(h5_filename, 'r') as h5_file:
            ...
            with plotting.Plotting(h5_filename) as p:
                ...
                VTh_fine_start = p.run_config['VTh_fine_start']
                VTh_fine_stop = p.run_config['VTh_fine_stop']
                ...
                p.plot_scurves(scurve_hist, range(VTh_fine_start, VTh_fine_stop), scan_parameter_name="VTh_fine", max_occ=max_occ)
                ...
                p.plot_distribution(hist, plot_range=range(VTh_fine_start, VTh_fine_stop), x_axis_title='VTh_fine', title='Threshold distribution', suffix='threshold_distribution')

                p.plot_occupancy(hist, z_label='Threshold', title='Threshold', show_sum=False, suffix='threshold_map', z_min=VTh_fine_start, z_max=VTh_fine_stop)
