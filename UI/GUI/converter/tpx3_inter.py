...
def raw_data_to_dut(raw_data, chip_links):
    '''
    ...
    k = (raw_data & 0xffffff)
    data_words = [np.empty(0, dtype=np.uint64)]*len(chip_links) # empty list element to store the final data_words
...
def interpret_raw_data(raw_data, chip_links):
    '''
    Chunk the data based on scan_param and interpret
    '''
    header = [[]]*len(chip_links)
    x      = [[]]*len(chip_links)
    y      = [[]]*len(chip_links)
    tot    = [[]]*len(chip_links)

    data_words = raw_data_to_dut(raw_data, chip_links)
    
    for chip in range(len(chip_links)):
        header[chip], x[chip], y[chip], tot[chip] = _interpret_raw_data(data_words[chip])
        #print('Interpreted data: ' + str(header[chip]) + ' ' + str(x[chip]) + ' ' + str(y[chip])+ ' ' + str(tot[chip]))
...
class Tpx3(Transceiver):
    ...
    def interpret_data(self, data):
        ...
        header, x, y, tot = interpret_raw_data(data[0][1], self.chip_links)

        #print('In interpret_data --: ')
        #print('After interpreting')
        ...
        hits = [[]]*len(self.chip_links)

        for chip in range(len(self.chip_links)):
        ...
        for header_list in range(len(self.chip_links)):
        ...
        #print('In interpret_data -- hits: ' + str(hits))
