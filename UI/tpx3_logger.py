...
class mask_logger(object):
    ...
    def write_full_mask(full_mask, chipID=None, file_name=None):
        '''
        ...
        if mask == None:
            ...
            path = user_path + os.sep + mask + '.h5'
        '''
        mask_folder = os.path.join(os.path.join(os.path.expanduser('~'), 'Timepix3'), 'masks')

        if file_name == 'New':
            now        = datetime.now()
            today      = date.today()
            time_stamp = f'{today}_{now.hour:02d}-{now.minute:02d}-{now.second:02d}'
            file_name  = f'{chipID}_mask_{time_stamp}'
            full_path  = f'{mask_folder}{os.sep}{chipID}{os.sep}{file_name}.h5'
        else:
            full_path  = f'{mask_folder}{os.sep}{chipID}{os.sep}{file_name}'

        #delete last mask
        if os.path.isfile(full_path):
            with tb.open_file(full_path, 'a') as infile:
                infile.remove_node(infile.root.mask_matrix)

        #Saving the final matrix
        with tb.open_file(full_path, 'a') as out_file:
            out_file.create_carray(out_file.root, name='mask_matrix', title='Matrix mask', obj=full_mask)
        return True

    def get_mask(path = None):
        '''
        ...
        if path == None:
            # return zeros
            mask_matrix = np.zeros((256*256, ), dtype=bool)
            #print('returning default mask (zeros)')
...
class file_logger(object):
    ...
    def create_default_file():
        ...
        json.dump(TPX3_datalogger.config(), default_file)
...
class TPX3_data_logger(object):
    ...
    def __init__(self):
        ...
        self.config_keys = ['software_version', 'firmware_version', 'hardware_links',
                            ...
                            'Chip8_name', 'Chip9_name', 'Chip10_name', 'Chip11_name',
                            'Chip12_name', 'Chip13_name', 'Chip14_name', 'Chip15_name',
                            'Chip16_name', 'Chip17_name', 'Chip18_name', 'Chip19_name',
                            'Chip20_name', 'Chip21_name', 'Chip22_name', 'Chip23_name',
    ...
    def config(self):
        return {'software_version' : 'x.x',
                ...
                'links' : {'Link_0': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX0'},
                           'Link_1': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX1'},
                           'Link_2': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX2'},
                           'Link_3': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX3'},
                           'Link_4': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX4'},
                           'Link_5': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX5'},
                           'Link_6': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX6'},
                           'Link_7': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX7'},
                           'Link_8': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX8'}, 
                           'Link_9': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX9'}, 
                           'Link_10': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX10'}, 
                           'Link_11': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX11'}, 
                           'Link_12': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX12'}, 
                           'Link_13': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX13'}, 
                           'Link_14': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX14'}, 
                           'Link_15': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX15'}, 
                           'Link_16': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX16'}, 
                           'Link_17': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX17'}, 
                           'Link_18': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX18'}, 
                           'Link_19': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX19'}, 
                           'Link_20': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX20'}, 
                           'Link_21': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX21'}, 
                           'Link_22': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX22'}, 
                           'Link_23': {'chip-id': None, 'chip-id-decoded': None, 'fpga-link': 0, 'chip-link': 0, 'data-delay': 0, 'data-invert': 0, 'data-edge': 0, 'link-status': 0, 'name': 'RX23'}},
    ...
    def update_chip_links(self):
        ...
        for n, info in enumerate(link_config):
            if link_config[info]['chip-id-decoded'] not in new_config:
                new_config[link_config[info]['chip-id-decoded']] = [n]
            else:
                new_config[link_config[info]['chip-id-decoded']].append(n)
            
        self.data['chip_links'] = new_config

    def update_links(self, link_config):
        chip_name         = link_config[0][0]
        chip_name_decoded = link_config[0][1]
        ...
        for link in configs:
            ...
            self.data['links'][label]['chip-id-decoded'] = chip_name_decoded
        ...
        for i in range(24):
            label = f'Link_{i}'
            if link_config[0][0] == self.data['links'][label]['chip-id']:
                for new_link in configs:
                    link_found_for_chip = False
                    if self.data['links'][label]['fpga-link'] == new_link[0]:
                        link_found_for_chip = True
                        break
                if link_found_for_chip != True:
                    self.data['links'][label]['chip-id']         = 0
                    self.data['links'][label]['chip-id-decoded'] = None
                    self.data['links'][label]['chip-link']       = 0
                    self.data['links'][label]['data-delay']      = 0
                    self.data['links'][label]['data-invert']     = 0
                    self.data['links'][label]['data-edge']       = 0
                    self.data['links'][label]['link-status']     = 0
        ...
        if self.name_valid(name) == True:
            if name in ['Chip0_name', 'Chip1_name', 'Chip2_name', 'Chip3_name',
                ...
                if name in self.general_config_keys:
                    if name in ['Equalisation_path', 'Mask_path']:
                        # In this case value is a list with two elements: 0 - path, 1 - chipID
                        # The logger logs a list of files
                        if self.data[name][value[1]] == None:
                            self.data[name][value[1]]['available'] = [value[0]]
                        else:
                            self.data[name][value[1]]['available'].append(value[0])
                        self.data[name][value[1]]['active'] = value[0]
                    else:
                        self.data[name] = value
                else:
                    if chip == None:
    ...
    def set_data(self, config):
        ...
        # Write something to validate the data, if the structure is correct
        try:
            self.data = config
            return True
        except:
    ...
    def get_dacs_from_yaml(self, chipID = None):
        '''
        ...
        for file in file_names:
            ...
            for chip in yaml_data['chips']:
                if chipID == None:
                    ...
                    if chip['chip_ID_decoded'] in chipID:
                        chip_name = chip['chip_ID_decoded']
                    else:
                        continue
    ...
    def write_backup_to_yaml(self):
        ...
        default_file_list = ['dacs.yml', 'outputBlock.yml', 'GeneralConfiguration.yml', 'PLLConfig.yml']
        chip_file_list    = ['chip_dacs.yml', 'chip_outputBlock.yml', 'chip_GeneralConfiguration.yml', 'chip_PLLConfig.yml']
    ...
    def check_mask_equal(self):
        '''
            A helper function, which checks, if there are folders created for mask
            and equalisation files for all chips, which are currently in the logger.
        '''
        user_path  = os.path.expanduser('~')
        user_path  = os.path.join(user_path, 'Timepix3')
        ...
        # Get list of chips
        chip_list = list(self.data['chip_dacs'])
    
        for chip in chip_list:
            chip_equal_path = os.path.join(equal_path, chip)
            chip_mask_path  = os.path.join(mask_path, chip)

            # Check, if folders are created
            if os.path.isdir(chip_equal_path) == False:
                os.mkdir(chip_equal_path)
            if os.path.isdir(chip_mask_path) == False:
                os.mkdir(chip_mask_path)

            # Create a list of files
            equal_file_list = os.listdir(chip_equal_path)  
            mask_file_list  = os.listdir(chip_mask_path)

            # Put the lists in the logger
            if equal_file_list == []:
                self.data['Equalisation_path'][chip] = {'available': ['None'], 'active': 'None'}
            else:
                # Append full path to files
                for n, file in enumerate(equal_file_list):
                    equal_file_list[n] = os.path.join(chip_equal_path, file)
                equal_file_list.append('None')
                # If the chip is not in list, create new entry with first file as active
                if chip not in list(self.data['Equalisation_path']):
                    self.data['Equalisation_path'][chip] = {'available': equal_file_list, 'active': equal_file_list[0]}
                else:
                    for file in equal_file_list:
                        # only append to 'available', active should be the same
                        if file not in self.data['Equalisation_path'][chip]['available']:
                            self.data['Equalisation_path'][chip]['available'].append(file)
                        # if active file is not in directory anymore, set to None
                        if self.data['Equalisation_path'][chip]['active'] not in equal_file_list:
                            self.data['Equalisation_path'][chip]['active'] = 'None'
                    # Set 'None' as last item
                    self.data['Equalisation_path'][chip]['available'].remove('None')
                    self.data['Equalisation_path'][chip]['available'].append('None')      


            if mask_file_list == []:
                self.data['Mask_path'][chip] = {'available': ['None'], 'active': 'None'}
            else:
                for n, file in enumerate(mask_file_list):
                    mask_file_list[n] = os.path.join(chip_mask_path, file)
                mask_file_list.append('None')
                if chip not in list(self.data['Mask_path']):
                    self.data['Mask_path'][chip] = {'available': mask_file_list, 'active': mask_file_list[0]}
                else:
                    for file in mask_file_list:
                        if file not in self.data['Mask_path'][chip]['available']:
                            self.data['Mask_path'][chip]['available'].append(file)
                        if self.data['Mask_path'][chip]['active'] not in mask_file_list:
                            self.data['Mask_path'][chip]['active'] = 'None'
                    self.data['Mask_path'][chip]['available'].remove('None')
                    self.data['Mask_path'][chip]['available'].append('None')             
