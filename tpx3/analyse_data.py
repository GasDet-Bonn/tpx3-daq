...
class DataAnalysis(ScanBase):
    ...
    def cluster(self, data, radius, dt, progress = None):
        cluster_nr = 0
        data_type = [('cluster_nr','uint64'), ('hits','int'), ('centerX','int'), ('centerY','int'), ('sumTOT','int'), ('sumCharge','int'), ('x','object'), ('y','object'), ('TOA','object'), ('TOT','object'), ('EventCounter','object'), ('chunk_start_time','double'), ('TOA_Extension','object')]
                    ...
                    while k < len(event_index):
                        ...
                        if (k == len(event_index) or stop == True) and len(frame_next):
                            ...
                            while f < len(event_index_next):
                                j = event_index_next[f]
                                # stop if TOA does not fit, as is is impossible to shift over two chunks
                                if np.abs(frame_next['TOA_Combined'][j].astype('int64')-toa_list[current])>dt:
                                    s += k+f
                                    n+=1
                                    break
                                # check whether it fits in space and add to cluster if so
                                if np.abs(frame_next['x'][j].astype('int64')-x_list[current])<=radius and np.abs(frame_next['y'][j].astype('int64')-y_list[current])<=radius:
                                    x_list.append(frame_next['x'][j])
                                    y_list.append(frame_next['y'][j])
                                    toa_list.append(frame_next['TOA_Combined'][j])
                                    tot_list.append(frame_next['TOT'][j])
                                    event_list.append(frame_next['EventCounter'][j])
                                    extension_list.append(frame_next['TOA_Extension'][j])
    ...
    def analyze(self, file_name, big = False, cluster_radius = 1.1, cluster_dt = 50, progress = None):
        ...
        with tb.open_file(self.h5_filename, 'r+') as h5_file:
            ...
            for num, i in enumerate(iteration_array):
                ...
                vlarray = h5_file.create_vlarray(group, 'TOA', tb.Int32Atom(shape=()), "TOA-values", filters=tb.Filters(complib='zlib', complevel=5))
                for i in range(cluster_data.shape[0]):
    ...
    def plot(self,file_name):
        ...
        with tb.open_file(file_name, 'r+') as h5_file:
            with plotting.Plotting(file_name) as p:
                ...
                p.plot_distribution(hist_size, plot_range = np.arange(-0.5, np.median(hist_size)*7+0.5, 1), x_axis_title='Cluster Size', y_axis_title='# of clusters', title='Size distribution', suffix='Size_distribution', fit=False)
                ...
                p.plot_distribution(hist, plot_range = np.arange(np.amin(hist)-0.5, np.median(hist) *7, 500), x_axis_title='Number of electrons per pixel', y_axis_title='# of pixels', title='Number of electrons per pixel', suffix='Number of electrons per pixel', fit=False)
...
if __name__ == "__main__":
    ...
    datafile = user_path + os.sep +datafile
    # analyze and plot
