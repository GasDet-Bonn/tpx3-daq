...
class Preparation():    
    ...
    """
    def generate_trigger_from_DUT1(self,filename, output_file_name, min_size = 2, min_charge = 0):
        hist_size = []
        first = True
        ...
        select_index = np.zeros(hits.shape[0], dtype = np.int8)
    ...
    """
    def assign_event_number_by_toa(self, filename, output_file_name, toa_width = 10000):
...
if __name__ == "__main__":
    ...
    file_names = ["data_take_2021-01-15_15-49-11.h5","data_take_2021-01-15_15-49-11.h5"]

    input_path = '~'
    input_path = os.path.expanduser(input_path)
    input_path = os.path.join(input_path, 'Timepix3')
    input_path = os.path.join(input_path, 'scans')
    input_path = os.path.join(input_path, 'hdf')
    datafiles = [input_path + os.sep +file_name for file_name in file_names]
    ...
    #prep.convert_to_silab_format(datafile, trigger_list)